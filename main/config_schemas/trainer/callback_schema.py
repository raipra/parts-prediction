...
class ModelCheckpointConfig:
    filename: str = "{epoch}-{Val_F1_Score:.2f}"
...
def setup_config() -> None:
    ...
    cs.store(name="best_model_checkpoint_schema", group="trainer/callbacks", node=BestModelCheckpointConfig)
    cs.store(name="last_model_checkpoint_schema", group="trainer/callbacks", node=LastModelCheckpointConfig)
    cs.store(name="learning_rate_monitor_schema", group="rainer/callbacks", node=LearningRateMonitorConfig)
